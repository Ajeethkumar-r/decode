...
const exports_ = `
export { UICtx, VOICES, CONCEPTS, BADGES, TIER_NAMES, THEME_PACKS, buildPalette, makeStyles, resolveFonts,
...
for (const voice of ["real", "arcade"]) {
  for (const themeKey of ["duo", "notion", "midnight", "calm"]) {
    const V = M.VOICES[voice];
    const TH = M.THEME_PACKS[themeKey];
    const F = M.resolveFonts(TH, "default");
    const T = M.buildPalette(themeKey);
    const mode = TH.dark ? "dark" : "light";
    for (const mobile of [false, true]) {
      const S = M.makeStyles(T, F, TH, mobile);
      const wrap = (el) => React.createElement(M.UICtx.Provider, { value: { T, S, F, V, TH, mode, mobile } }, el);
      ...
      const tag = `${voice}/${themeKey}/${mobile ? "mobile" : "desktop"}`;

      t(`${tag} Login`, wrap(React.createElement(M.Login, { defaultVoice: voice, onDone: noop, onGoogle: noop, theme: themeKey, setTheme: noop })));
      ...
      t(`${tag} CaseBoard`, wrap(React.createElement(M.CaseBoard, { energy: 4, dailyDone: false, dailyQuest: daily, history, bossReady: true, onPick: noop, onBoss: noop, voice, quizDone: false, gameDone: false, onQuiz: noop, onGame: noop, topicStars: ["takeaway"], onSelfTest: noop, storyProgress: 1, formulaDone: false, onFormula: noop, storyItems: ["🥪"], onStoryItem: noop, storyOutro: "test outro" })));
      ...
      t(`${tag} Profile`, wrap(React.createElement(M.Profile, { profile, setProfile: noop, voice, setVoice: noop, theme: themeKey, setTheme: noop, fontChoice: "default", setFontChoice: noop, xp: 500, activeTitle: "Novice", earnedBadges: ["first"], topicStars: [], bankDone: [], onShare: noop, onSignOut: noop })));
